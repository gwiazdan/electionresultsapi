...
public interface MunicipalityRepository extends JpaRepository<Municipality, Long> {
    @Query("SELECT new com.norbigigakoks.electionresults.dto.CountyVoteSummary(c.countyID, c.name, SUM(m.numberOfVotes), SUM(m.votesForKO), SUM(m.votesForPIS), SUM(m.votesForKONF), SUM(m.votesForTD), SUM(m.votesForLEW), SUM(m.votesForBS), SUM(m.votesForMN), c.senate) " +
